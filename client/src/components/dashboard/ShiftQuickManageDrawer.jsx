...
import { updateShiftAsync, fetchAllShiftsAsync } from "../../store/feature/shiftsSlice";
...
export function ShiftQuickManageDrawer({ open, onClose }) {
  ...
  const toggleShift = async (dayKey, period) => {
    ...
    const current = Boolean(userShifts.shifts?.[dayKey]?.[period]);
    setBusy(true);
    try {
      await dispatch(
        updateShiftAsync({
        ...
        {userShifts && (
          <div className="flex flex-col gap-2">
            {WEEK_DAYS.map(({ key, labelIt }) => {
              const day = userShifts.shifts?.[key] || {};
              return (
                <div
                  key={key}
                ...
                >
                  ...
                  <button
                    type="button"
                    disabled={busy}
                    className={`custom-button text-xs ${day.morning ? "" : "custom-button-light"}`}
                    onClick={() => toggleShift(key, "morning")}
                  >
                    {t("shiftMorning")} {day.morning ? "✓" : "—"}
                  </button>
                  <button
                    type="button"
                    disabled={busy}
                    className={`custom-button text-xs ${day.afternoon ? "" : "custom-button-light"}`}
                    onClick={() => toggleShift(key, "afternoon")}
                  >
                    {t("shiftAfternoon")} {day.afternoon ? "✓" : "—"}
                  </button>
                </div>
              );
            })}
