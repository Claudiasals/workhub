...
export function normalizeCustomerOrders(orders = []) {
  return [...orders]
    .map((order) => ({
      ...
      locationName: order.pointOfSales?.name || "",
    }))
...
export function computePurchaseFrequencyDays(sortedOrders) {
  ...
  for (let i = 1; i < sortedOrders.length; i++) {
    gaps.push(Math.round((sortedOrders[i].created - sortedOrders[i - 1].created) / MS_DAY));
  }
  return Math.round(gaps.reduce((a, b) => a + b, 0) / gaps.length);
}

function countByCategory(orders) {
...
export function computeCustomerMetrics(customer, orders = []) {
  ...
  if (tier === "premium" || totalOrders >= 10 || points >= 500) loyaltyLevel = "Alto";
  else if (totalOrders >= 3 || points >= 100) loyaltyLevel = "Medio";

  let profileLabel = "Cliente occasionale";
  if (tier === "premium" || totalOrders >= 10) profileLabel = "Cliente fidelizzato";
  else if (totalOrders >= 3) profileLabel = "Cliente ricorrente";
